...
// ── Club / Group Select ────────────────────────────────────────────────────────
// Dropdown with existing values + "Autre…" option to type a new one
...
function ClubGroupSelect({
  ...
  // "custom" mode when value isn't empty AND not in the known list
  const isCustom = value !== "" && !suggestions.includes(value);
  const [custom, setCustom] = useState(isCustom);

  // Keep custom mode in sync when parent resets (e.g. dialog reopens)
  useEffect(() => {
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="sm:max-w-[360px]">
          ...
          <div className="grid gap-4 py-2">
            ...
            <div className="grid gap-2">
              <Label>Club</Label>
              <ClubGroupSelect id="edit-club" value={club} onChange={setClub} placeholder="ex: AC Bordeaux" suggestions={clubs} />
            </div>
            <div className="grid gap-2">
              <Label>Groupe</Label>
              <ClubGroupSelect id="edit-group" value={group} onChange={setGroup} placeholder="ex: Seniors" suggestions={groups} />
            </div>
          </div>
          <DialogFooter>
