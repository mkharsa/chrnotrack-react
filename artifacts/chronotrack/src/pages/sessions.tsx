...
import { useListSessions, useBulkDeleteSessions, getListSessionsQueryKey, useCreateSession, useListParticipants } from "@workspace/api-client-react";
import { format } from "date-fns";
import { Link, useLocation } from "wouter";
import { Button } from "@/components/ui/button";
import { Checkbox } from "@/components/ui/checkbox";
import { Plus, Trash2, Users, Calendar as CalendarIcon, ChevronRight } from "lucide-react";
    ...
    <div className="flex flex-col h-full relative">
      <div className="flex-1 p-4 overflow-y-auto">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-bold tracking-tight text-foreground uppercase">All Sessions</h2>
          <CreateSessionDialog />
        </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-4 text-xs text-muted-foreground">
                    <div className="flex items-center gap-1">
                      ...
                      <span>{session.participantCount} athletes</span>
                    </div>
                    {session.defaultDist && (
      ...
      <DialogContent className="sm:max-w-[425px]">
        ...
        <div className="grid gap-4 py-4">
          <div className="grid gap-2">
            <Label htmlFor="name">Session Name (e.g. 10x400m)</Label>
          ...
          <div className="grid gap-2 mt-2">
            ...
            <div className="max-h-[200px] overflow-y-auto space-y-2 border border-border rounded-md p-2">
              {participants?.map(p => (
              ...
              {participants?.length === 0 && (
                <div className="text-sm text-muted-foreground p-2">No existing athletes. You can add them later.</div>
              )}
            </div>
          </div>
        </div>
